...
if (isset($_POST['username'])
    ...
    if ($conn->connect_error) {
        ...
        if ($result->num_rows > 0)
        {
            ...
            $sql = "INSERT INTO user (email, username, password, firstname, lastname, gender)".
                " VALUES('{$email}','{$uname}','{$upass}','{$firstname}','{$lastname}', {$gender});";

            if($conn->query($sql) === TRUE) {

                $sql = "INSERT INTO profile (userID, seeking)".
                    " VALUES((SELECT id FROM user where userName = '{$uname}'), '{$seeking}');";
                if($conn->query($sql) === TRUE) {
                    try {
                        $semail = new Email($email, $firstname." ".$lastname);
